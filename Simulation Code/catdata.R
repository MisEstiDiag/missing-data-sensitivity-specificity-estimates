#ACD: Analysis of categorical data with missing (or complete) responses via product-multinomial distributions
#Version: 0.0-4
#Date: 2012/05/30
#Author: Frederico Zanqueta Poleto, Julio da Motta Singer and Carlos Daniel Paulino
...
summary.readCatdata<-function(x, digits=max(3,getOption("digits")-3)) {
  cat("\nCall: ",deparse(x$call),"\n\n",sep="")
  
  cat("\nS=",x$S," subpopulations  x  R=",x$R," response categories  with ",ifelse(x$miss,"MISSING","COMPLETE")," data\n",sep="")
  
  cat("\n\nTable of frequencies",ifelse(x$miss," of the complete data",""),":\n",sep="")
  print.default(x$TF[1:x$S,1:x$R],quote=FALSE,print.gap=2)
  ...
  if(x$miss) {
    ...
    for(s in 1:x$S) {
      if(x$Tt[s]>1) {
        ...
        for(tt in 2:x$Tt[s]) {
          cat("\n")
          zz<-t(rbind(x$Nst[[paste("st",s,".",tt,sep="")]], rep("",x$Rp[s,tt-1]), rep("",x$Rp[s,tt-1]), x$Zst[[paste("st",s,".",tt,sep="")]]))
          colnames(zz)<-c(rep("",3),paste("[,",1:x$R,"]",sep=""))
...
summary.satMarML<-function(x, digits=max(3,getOption("digits")-3)) {
  cat("\nCall: ",deparse(x$call),"\n\n",sep="")
  ...
  if(x$conv==FALSE) {
    cat("\n\n",x$method," has NOT attained the convergence criterion with ",x$maxit," iterations.\n\n",sep="")
  } else {
    cat("\n\n",x$method," attained the convergence criterion in ",x$it," iterations.\n\n",sep="")
  }
  ...
  cat("\n\nAugmented estimated frequencies under ",x$missing,":\n",sep="")
  for(s in 1:x$S) {
    if(x$S>1) {
      cat("\nSubpopulation ",s,"\n",sep="")
    }
    pp<-matrix(0,x$Tt[s],x$R)
    pp[1,]<-x$yst[[paste("st",s,".",1,sep="")]]
    if(x$Tt[s]>1) {
      for(tt in 2:x$Tt[s]) {
        pp[tt,]<-x$yst[[paste("st",s,".",tt,sep="")]]
      }
    }
    print.default(format(round(pp,digits=digits),digits=digits),quote=FALSE,print.gap=2,right=TRUE)
  }
...
summary.satMcarWLS<-function(x, digits=max(3,getOption("digits")-3)) {
  cat("\nCall: ",deparse(x$call),"\n\n",sep="")
...
linML<-function(obj,A=NULL,X=NULL,U=NULL,start=NULL,maxit=100,trace=0,epsilon1=1e-6,epsilon2=1e-6,zeroN=NULL, digits=max(3,getOption("digits")-3)) {
  ...
  #Obtem a matriz U (ou X) a partir da X (ou U), via decomposicao valor singular de uma matriz Q
  ...
  #Fisher information matrix i_1(\bar{theta})
  #>>>>>>>>>> Esta informacao de Fisher e diferente da existente em satMarML !!!
  #Aquela estima iterativamente alphast e esta utiliza sempre o alphast obtido de theta sem restricoes
...
summary.linML<-function(x, digits=max(3,getOption("digits")-3)) {
  cat("\nCall: ",deparse(x$call),"\n\n",sep="")
  
  pp<-matrix(0,x$S,x$R)
  epp<-matrix(0,x$S,x$R)
  for(s in 1:x$S) {
    pp[s,]<-x$thetaH[((s-1)*x$R+1):(s*x$R)]
    epp[s,]<-sqrt(diag(x$VthetaH[((s-1)*x$R+1):(s*x$R),((s-1)*x$R+1):(s*x$R)]))
  }
  ...
  pp<-matrix(0,x$u,4)
  pp[,1]<-x$Fu
  pp[,2]<-sqrt(diag(as.matrix(x$VFu)))
  pp[,3]<-x$FH
  pp[,4]<-sqrt(diag(as.matrix(x$VFH)))
  ...
  if(x$conv==FALSE) {
    cat("\n\nFisher scoring has NOT attained the convergence criterion with ",x$maxit," iterations.\n",sep="")
  } else {
    cat("\n\nFisher scoring attained the convergence criterion in ",x$it," iterations.\n",sep="")
  }
  ...
  if(x$miss==TRUE) {
    cat("\n\nAugmented estimated frequencies under the linear model and ",x$missing,":\n",sep="")
    for(s in 1:x$S) {
      if(x$S>1) {
        cat("\nSubpopulation ",s,"\n",sep="")
      }
...
summary.loglinML<-function(x, digits=max(3,getOption("digits")-3)) {
  cat("\nCall: ",deparse(x$call),"\n\n",sep="")
  ...
  if(x$miss==TRUE) {
    cat("\n\nAugmented estimated frequencies under log-linear model and ",x$missing,":\n",sep="")
    for(s in 1:x$S) {
      if(x$S>1) {
        cat("\nSubpopulation ",s,"\n",sep="")
      }
...
funlinWLS<-function(model,obj=NULL,theta=NULL,Vtheta=NULL,A1=NULL,A2=NULL,A3=NULL,A4=NULL,A5=NULL,A6=NULL,A7=NULL,A8=NULL,A9=NULL,X=NULL,U=NULL,XL=NULL,UL=NULL,zeroN=NULL,PI1=NULL,PI2=NULL,PI3=NULL,PI4=NULL,PI5=NULL,PI6=NULL,PI7=NULL,PI8=NULL,PI9=NULL) {
...
summary.funlinWLS<-function(x, digits=max(3,getOption("digits")-3)) {
  cat("\nCall: ",deparse(x$call),"\n",sep="")
  
  if(x$type=="linear" | x$type=="log-linear") {
    pp<-matrix(0,x$S,x$R)
    epp<-matrix(0,x$S,x$R)
    for(s in 1:x$S) {
      pp[s,]<-x$thetaH[((s-1)*x$R+1):(s*x$R)]
...
print.waldTest<-summary.waldTest<-function(x, digits=max(3,getOption("digits")-3)) {
  cat("\nCall: ",deparse(x$call),"\n",sep="")
  
  cat("\nWald statistic of the hypothesis (d.f.=",x$glH,"): ",
      round(x$QwH,digits=digits)," (p-value=",
      ifelse(x$glH>0,format(round(1-pchisq(x$QwH,x$glH),digits=digits),digits=digits),1),")\n",sep="")
